...
norm_pos = hist_pos / np.sum(hist_pos)
hist_neg, _ = np.histogram(vec_m6A[vec_dge < -thresh_dge], range=[-bin_max, bin_max], bins=num_bins)
norm_neg = hist_neg / np.sum(hist_neg)
plt.plot(bin_centers, norm_pos, label=f'DGE positive')
plt.plot(bin_centers, norm_neg, label=f'DGE negative')
plt.legend()
...
plt.subplot(2, 2, 2)
plt.bar(bin_centers, norm_pos-norm_neg, width=bin_width)
