...
def plot_distribution(vec_mod, vec_mask, thresh_mask, mod_name):
...
def plot_prob_diff(norm_pos, norm_neg, mod_name):
...
plt.subplot(2, 2, 1)
norm_pos_m6A, norm_neg_m6A = plot_distribution(vec_m6A, vec_dge, thresh_dge, 'm6A')
plt.subplot(2, 2, 2)
plot_prob_diff(norm_pos_m6A, norm_neg_m6A, 'm6A')
plt.subplot(2, 2, 3)
norm_pos_psi, norm_neg_psi = plot_distribution(vec_psi, vec_dge, thresh_dge, 'psi')
plt.subplot(2, 2, 4)
plot_prob_diff(norm_pos_psi, norm_neg_psi, 'psi')
# plt.suptitle(ds)
plt.savefig(os.path.join(img_out, f'hist_DGE_vs_delta_logit_S_{ds}_FDR{thresh_fdr}_pval{thresh_pval}.{FMT}'), **fig_kwargs)
