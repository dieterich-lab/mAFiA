...
for MOD in $ALL_MODS
do
  OUTDIR=${WORKSPACE}/${ORIG}_${ALL_RUNS/ /_}_${MOD}
...
### merge A and m6A ###########################################################
###############################################################################
MERGE_DIR=${WORKSPACE}/${ORIG}_${ALL_RUNS/ /_}_${ALL_MODS/ /_}
MERGE_BAM_INPUTS=""
...
do
  ### fast5 ###
  for f in ${WORKSPACE}/${ORIG}_${ALL_RUNS/ /_}_${MOD}/fast5/*.fast5
  do
    ln -s ${f}
  done

  MERGE_BAM_INPUTS+="${WORKSPACE}/${ORIG}_${ALL_RUNS/ /_}_${MOD}/spomelette_q70.bam "

  MERGE_REF_INPUTS+="${WORKSPACE}/${ORIG}_${ALL_RUNS/ /_}_${MOD}/ligation_ref.fasta "
done
