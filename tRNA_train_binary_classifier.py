...
for ind, mod_site in df_mod_sel.iterrows():
    ...
    if (len(wt_site_motif_features)>min_coverage) and (len(ivt_site_motif_features)>min_coverage):
        ...
        if classifier=='svm':
            ...
            auc_score, classifier_model, opt_thresh = train_binary_classifier(ivt_site_motif_features, wt_site_motif_features, ref_motif, classifier, scaler=scaler, debug_img_dir=os.path.join(classifier_model_dir, 'auc'))
        else:
